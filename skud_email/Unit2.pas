...
begin
  ...
  ShowMessage('Такой работник уже добавлен.');
end;
...
begin
  try
    ...
    FIniFile := TINIFile.Create(ExtractFilePath(Application.ExeName) + 'Config.ini');
    EmailList:=FIniFile.ReadString('Email','address','');
    try
...
begin
  ...
  begin
    ...
    begin
      ...
      if (IBQuery5.FieldByName('DT').AsString <> IBQuery4.Fields[1].AsString) then //если записи разные то
      begin
        ...
        IdMessage1.Recipients.EMailAddresses:=EmailList;
